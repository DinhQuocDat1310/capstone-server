export const policies: { question: string; answer: string }[] = [
  {
    question: 'Company Governance',
    answer:
      "How management leads a company is crucial to employee motivation, addressing concerns, and planning the organization's goals. Since people make up businesses, it's crucial for employees to know what their leaders believe in and stand for, as well as, what it means to be a part of the company",
  },
  {
    question: 'Employee Conduct',
    answer:
      'Policies regarding employee conduct layout the specific responsibilities and tasks that all workers must follow and do as part of their job. These rules establish guidelines for employee behavior including: Dress code, Safety procedures, Harassment policies, Technology use, Discipline, Appropriate behavior, Warnings, Termination',
  },
  {
    question: 'Employee Position Descriptions',
    answer:
      'Job descriptions clarify the role of every worker including their responsibilities, amount of authority, overall goals, and specific duties. Position descriptions create a clear-cut system for employee training and employee performance assessment.',
  },
  {
    question: 'Personnel Policies / Compensation and Benefits Policies',
    answer:
      'Personnel guidelines clarify things like: Business hours, Employment terms, Wages and salary, Bonuses, Insurance and benefits, Vacations, Sick days, Retirement',
  },
  {
    question: 'Organizational Structure',
    answer:
      "This is a chart with every employee's name and job title that illustrates every employee's place in the organization's structure",
  },
  {
    question: 'Equal Opportunity/ Non-Discrimination/Harassment/Retaliation',
    answer:
      'These rules ensure that employees are treated fairly at work. Most businesses have equal opportunity policies, including discrimination and affirmative action rules, that encourage fair treatment for all employees.       Every organization should offer equal opportunity employment to keep employees from facing inappropriate behavior from other workers, management, and contractors regarding the following: Race, Sexual orientation, Creed, Gender, Cultural beliefs, Religious beliefs, Every company should have guidelines in place concerning disabled or pregnant applicants or employees. Diversity guidelines should be in place. All organizations need a "no retaliation" policy to protect workers that report instances of harassment or discrimination',
  },
];

export const faq: { question: string; answer: string }[] = [
  {
    question: 'Who are your drivers? Are they only rideshare?',
    answer:
      'Our 420,000+ contracted drivers consist of rideshare drivers on platforms like Uber and Lyft, app-enabled deliver drivers for platforms like Doordash, Grubhub, Uber Eats and Instacart, as well as everyday commuters who want to earn money for the miles they drive.',
  },
  {
    question: 'What markets can we launch a campaign?',
    answer: 'Brandvertise is live nationwide!',
  },
  {
    question: 'How much does it cost to run a campaign',
    answer: 'Each Brandvertise campaign varies in size, scale and scope.',
  },
  {
    question:
      'How do we ensure design consistency on all the various makes and models?',
    answer:
      'The Brandvertise creative team works as an extension of your brand or agency team to ensure consistency across all makes and models of vehicles. We work with certified installers to ensure your brand creative looks flawless.',
  },
  {
    question: 'Is production media and installation all included?',
    answer:
      'Yes, production media and installation are included with every Brandvertise campaign.',
  },
];

export const terms: { question: string; answer: string }[] = [
  {
    question: 'GENERAL',
    answer:
      'These terms and conditions will take effect from the date of issuance and will be binding on the driver at the time of acceptance on the system.',
  },
  {
    question: 'RIGHT OF BRANDVERTISE',
    answer:
      'Brandvertise reserves the right to request that the DRIVER provide accurate information and documents related to the DRIVER, including personal data such as their full name, email, phone number, and account number. In the event that DRIVER breaches its obligations set out in these Terms and Conditions, Brandvertise has the right to suspend the provision of the Application Services after Brandvertise sends a written notice to DRIVER and claims damages (if any).',
  },
  {
    question: 'RIGHT OF DRIVER',
    answer:
      'DRIVER reserves the right to require Brandvertise to provide application services to Brandvertise in accordance with the terms of the agreement',
  },
  {
    question: 'OBLIGATIONS OF BRANDVERTISE',
    answer:
      'Brandvertise will provide application services to the driver in accordance with the terms and conditions of the agreement. ',
  },
  {
    question: 'OBLIGATIONS OF DRIVER',
    answer:
      "When a campaign in which the driver participated is wrapping, Driver must come to checkpoint at wrapping date to wrap driver car when join campaign.       When a campaign in which the driver participated is running, Driver must drive their car to check point every day when campaign start running.   When a campaign in which the driver participated is running, Driver must enable tracking the driver's location while driving.",
  },
];
